...
if(!empty($_SESSION['kodeakses'])) {   
    ...
    $pembayaran = $config->getData("SELECT * FROM tbl_pembayaran_isedentil WHERE id_pembayaran_isedentil='". $jenis_pembayaran ."'");
    ...
    if($result==false) {
        echo ("<script LANGUAGE='JavaScript'>
                ...
                window.location.href='". $base_url ."page/transaksi-isedentil';
                </script>");
    }
    ...
    <div class="page-content-wrap">
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="panel panel-primary">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9 col-xs-12"> 
                                <input type="text" class="form-control" style="background-color: white; color: black;" name="id_transaksi_isedentil" placeholder="Masukan ID Transaksi" required="" readonly="" value="<?= $config->kode_transaksi_isedentil()?>" />    
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9 col-xs-12">
                                <input type="hidden" class="form-control" name="id_pembayaran_isedentil" placeholder="Masukan Pembayaran" required="" value="<?= $p['id_pembayaran_isedentil']; ?>" /> 
                        ...
                        <div class="form-group hidden">
                            ...
                            <div class="col-md-3 col-xs-5">
                                <?php
                                    $kurang = $config->getData("SELECT SUM(nominal_transaksi) as nominal FROM tbl_transaksi_isedentil WHERE id_siswa='". $r['id_siswa'] ."' AND id_kelas = '". $kelas_siswa ."' AND id_pembayaran_isedentil='". $p['id_pembayaran_isedentil'] ."'");
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-3 col-xs-12"> 
                                ...
                                ?>
                                <input type="number" class="form-control" style="background-color: white; color: black;" name="jumlah_cicilan" placeholder="Masukan Jumlah Cicilan" required="" value="<?= $config->kode_cicilan_isedentil($siswa, $id_kelas, $idpembayaran)?>" readonly="" />    
                            </div>
                    ...
                    <div class="panel-footer">
                        <a href="<?= $base_url . 'page/transaksi-isedentil'; ?>" class="btn btn-default btn-lg ">Batal</a>
                    ...
                    <div class="panel-body table-responsive">
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <?php 
                                    $i=1;
                                    $histori = $config->getData("SELECT * FROM tbl_transaksi_isedentil t, tbl_siswa s, tbl_pegawai p WHERE t.id_kelas = '". $kelas_siswa ."' AND t.id_siswa = s.id_siswa AND t.id_pegawai = p.id_pegawai AND t.id_siswa='". $r['id_siswa'] ."' AND t.id_pembayaran_isedentil ='". $jenis_pembayaran ."'");
                                ...
                                <tr>
                                    ...
                                    <td align="center">
									<?php 
										$enidtransaksi1=base64_encode($h['id_transaksi_isedentil']);
                                        ...
                                        $id_kelas = $config->getData("SELECT id_kelas FROM tbl_transaksi_isedentil WHERE id_transaksi_isedentil='". $h['id_transaksi_isedentil'] ."'");
                                        foreach ($id_kelas as $ik) {
                                            $enidkelas=base64_encode(base64_encode(base64_encode($ik['id_kelas'])));
                                        }
                                        
										$enidpembayaran1=base64_encode($h['id_pembayaran_isedentil']);
                ...
                <div class="panel panel-primary">
                    <div class="panel-heading">
                        <h3 class="panel-title">Riwayat Pembayaran Dahulu</h3>
                    </div>
                    <div class="panel-body table-responsive">
                ...
                <div class="panel panel-primary">
                    <div class="panel-heading">
                        <h3 class="panel-title">Tunggakan Pembayaran</h3>
                    </div>
                    <div class="panel-body table-responsive">
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <?php 
                                    $i=1;
                                    $histori = $config->getData("SELECT *, t.id_kelas as tid FROM tbl_transaksi_isedentil t, tbl_siswa s, tbl_pegawai p, tbl_pembayaran_isedentil pm WHERE pm.id_pembayaran_isedentil = t.id_pembayaran_isedentil AND t.id_kelas <> '". $kelas_siswa ."' AND t.id_siswa = s.id_siswa AND t.id_pegawai = p.id_pegawai AND t.id_siswa='". $r['id_siswa'] ."' AND t.id_pembayaran_isedentil ='". $jenis_pembayaran ."' GROUP BY t.id_kelas");
                                ...
                                <tr>
                                    <td><?= $i; ?></td>
                                    <td><?= $h['nama_pembayaran']; ?> 

                                    <?php 
                                        $kelas = $config->getData("SELECT * FROM tbl_kelas WHERE id_kelas='". $h['tid'] ."'");
                                        foreach ($kelas as $k) {
                                            echo $config->format_romawi($k['tingkat_kelas']);
                                        }
                                    ?>
                                     </td>

                                <?php
                                    $transaksi = $config->getData("SELECT * FROM tbl_transaksi_isedentil WHERE id_transaksi_isedentil='". $h['id_transaksi_isedentil'] ."'");

                                    foreach ($transaksi as $t) {
                                        $id_kelas_transaksi = $t['id_kelas'];
                                    }

                                    $kurang = $config->getData("SELECT SUM(nominal_transaksi) as nominal FROM tbl_transaksi_isedentil WHERE id_siswa='". $r['id_siswa'] ."' AND id_kelas = '". $id_kelas_transaksi ."' AND id_pembayaran_isedentil='". $h['id_pembayaran_isedentil'] ."'");

                                    foreach ($kurang as $k) {
                                        $nilai_pembayaran = $h['nominal_pembayaran'];
                                        $pembayaran_kurang = $nilai_pembayaran  - $k['nominal'];
                                    }
                                ?>
                                    <td><?= $config->format_rupiah($pembayaran_kurang); ?></td>
                                </tr>
                                <?php
                                    $i++;
                                    }
