<?php
	...
	$id_transaksi_isedentil = "'" . $_POST['id_transaksi_isedentil'] . "'";
	$waktu_transaksi = date('Y-m-d H:i:s');
	$id_siswa = "'" . $config->escape_string($_POST['id_siswa']) . "'";
	$id_kelas = "'" . $config->escape_string($_POST['id_kelas']) . "'";
	$id_pegawai = $_SESSION['id_pegawai'];
	$id_pembayaran_isedentil = "'" . $config->escape_string($_POST['id_pembayaran_isedentil']) . "'";
...
if(isset($_POST['bayar'])) {
	if(empty($lokasi_file)) {
		$result = $config->execute("
			INSERT INTO tbl_transaksi_isedentil(
				id_transaksi_isedentil,
				waktu_transaksi,
				id_siswa,
				id_kelas,
				id_pegawai,
				id_pembayaran_isedentil,
				nominal_transaksi,
				pembayaran_melalui,
				file_foto,
				cicilan_transaksi) 
	...
	if($result) {
		...
		$id_transaksi_isedentil	= base64_encode(base64_encode(base64_encode($_POST['id_transaksi_isedentil'])));
		...
		$id_pembayaran_isedentil 	= base64_encode(base64_encode(base64_encode($_POST['id_pembayaran_isedentil'])));
		$cicilan 		= base64_encode(base64_encode(base64_encode($_POST['jumlah_cicilan'])));

		$_SESSION['cetak'] = "<script>window.open('cetak.php?id=". $id_transaksi_isedentil . "&siswa=". $id_siswa."&pembayaran=". $id_pembayaran_isedentil."&cicilan=". $cicilan. "&id_kelas=". $id_kelas. "', '_blank');</script>";
	} else {
	...
	$result = $config->execute("
		DELETE FROM tbl_pembayaran_isedentil WHERE id_pembayaran_isedentil='". $_GET['id'] ."'");
