...
function __backup_mysql_database($params)
{
    ...
    $backup_file_name = "SPP web ".date( "d-m-Y--h-i-s").".sql";
         
    $fp = fopen('backup/' . $backup_file_name ,'w+');
    if (($result = fwrite($fp, $contents))) {
        echo "<center><a href='backup/$backup_file_name' class='btn btn-danger btn-lg'>Download Database</a></center>"; 
    }
    fclose($fp);
