...
if(isset($_POST['simpan'])) {
	$result = $config->execute("
		INSERT INTO tbl_pembayaran_isedentil(
			nama_pembayaran,
			nominal_pembayaran,
			jumlah_cicilan,
			id_kelas,
	...
	$idj1 =base64_decode($_POST['id_pembayaran_isedentil']);
	$idj2 =base64_decode($idj1);
	$idj3 =base64_decode($idj2);
	$id_pembayaran_isedentil 	= "'" . $config->escape_string($idj3) . "'";

	$result = $config->execute("
		UPDATE tbl_pembayaran_isedentil SET 
		...
		WHERE 
			id_pembayaran_isedentil	= $id_pembayaran_isedentil
	");
	...
	{ 
	  ...
	  sleep(0);
		...
		$sql = "REPLACE INTO tbl_pembayaran_isedentil VALUES 
				(
				'".$rowData[0][0]."',
				'".$rowData[0][1]."',
				'".$rowData[0][2]."',
	...
	$result = $config->execute("
		DELETE FROM tbl_pembayaran_isedentil WHERE id_pembayaran_isedentil='". $id3 ."'");
