...
// Settings
$base_url 			= "http://localhost/adm2/";
$name_application 	= "MAN 2 PAYAKUMBUH";
$company			= "MAN 2 PAYAKUMBUH";
$address			= "PAYAKUMBUH";
$logo				= "assets/man2.jpg";
$copyright			= "";
$copyright_link		= "";
...
class Config extends DbConfig
{
	...
	public function getIsedentil($query)
	{		
		$result = $this->connection->query($query);
		
		$query = "SELECT * FROM tbl_transaksi_isedentil t, tbl_siswa s, tbl_pegawai p, tbl_pembayaran_isedentil pn, tbl_kelas k WHERE t.id_siswa = s.id_siswa AND t.id_pegawai = p.id_pegawai AND t.id_pembayaran_isedentil = pn.id_pembayaran_isedentil AND t.id_kelas = k.id_kelas ORDER BY t.id_transaksi_isedentil DESC ";
	...
	function kode_transaksi_isedentil(){          
		$query = $this->connection->query('SELECT RIGHT(id_transaksi_isedentil, 12) as kode FROM tbl_transaksi_isedentil ORDER BY id_transaksi_isedentil DESC limit 1');
		$kode = 1;    
		foreach ($query as $q) {
		    $kode = intval($q['kode']) + 1;
		}      
	...
	function kode_cicilan_isedentil($id_siswa, $id_kelas, $id_pembayaran_isedentil){          
		$query = $this->connection->query("SELECT count(*) as kode FROM tbl_transaksi_isedentil, tbl_siswa WHERE tbl_transaksi_isedentil.id_siswa = tbl_siswa.id_siswa AND tbl_transaksi_isedentil.id_siswa='$id_siswa' AND tbl_transaksi_isedentil.id_pembayaran_isedentil='$id_pembayaran_isedentil' AND tbl_transaksi_isedentil.id_kelas = '$id_kelas' AND tbl_transaksi_isedentil.id_kelas = tbl_siswa.id_kelas ORDER BY tbl_transaksi_isedentil.id_transaksi_isedentil DESC"); 
